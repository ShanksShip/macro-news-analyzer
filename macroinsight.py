...
with st.sidebar:
    st.markdown("## 设置")
    api_key = st.text_input("DeepSeek API Key", type="password", help="输入您的DeepSeek API密钥")
    ...
    st.info(
        ...
        "利用DeepSeek AI模型分析宏观经济新闻对金融市场的潜在影响。"
        "\n\n该工具提供对影响时长、方向、力度和消退指标的专业评估。"
    )

    st.markdown("### 使用指南")
    st.markdown(
        "1. 输入您的DeepSeek API密钥\n"
        "2. 粘贴宏观经济或金融新闻内容\n"
        "3. 点击'分析新闻'按钮\n"
        "4. 查看详细的多维度分析结果"
    )
...
# 分析按钮
if st.button("分析新闻", disabled=not (api_key and news_text)):
    if not api_key:
        st.error("请输入有效的DeepSeek API密钥")
    elif not news_text:
        st.error("请输入新闻内容")
    else:
        with st.spinner("正在分析新闻内容，这可能需要15-30秒..."):
            try:
                ...
                if response.status_code == 200:
                    ...
                    summary_response = requests.post(url, headers=headers, data=json.dumps(summary_data))
...
# 添加页脚
st.markdown('<div class="footer">© 2025 MacroInsight 宏观新闻分析工具 | 基于DeepSeek AI模型</div>',
            unsafe_allow_html=True)
